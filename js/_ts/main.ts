...
interface IInputCollection {
  ['input-day']: HTMLInputElement;
  ['input-month']: HTMLInputElement;
  ['input-year']: HTMLInputElement;
}

interface IOutputCollection {
  ['output-day']: HTMLOutputElement;
  ['output-month']: HTMLOutputElement;
  ['output-year']: HTMLOutputElement;
}

interface IFormCollection extends IInputCollection, IOutputCollection, HTMLFormControlsCollection {
  ['date-field']: HTMLFieldSetElement;
}

interface IMsgErrorProp {
...
interface IMsgError {
  date: IMsgErrorProp;
  input: {
    [key in keyof IInputCollection]: IMsgErrorProp;
  };
...
const MSG_ERROR: IMsgError = {
  ...
  input: {
    'input-day': {
      ...
      max: '', // The value is generated below
      min: 'Must be >= 1',
    },
    'input-month': {
...
const inputArr = Object.keys(MSG_ERROR.input) as Array<keyof IInputCollection>;
const outputArr: Array<keyof IOutputCollection> = ['output-day', 'output-month', 'output-year'];
const outputData = document.body.querySelector('.output-row') as HTMLDivElement;
...
let inputDateMin: Date;
let formDataPrev: string;
...
btnSubmit.addEventListener('click', (e): void => {
  ...
  let formData: IterableIterator<FormDataEntryValue> | string = new FormData(form).values();
  ...
  const formCollection = form.elements as IFormCollection;
  ...
  if (formValidity || wasError.input) {
    ...
    // Check all input fields
    inputArr.forEach((inputName): void => {
      ...
      if (!inputError.tag) {
        inputError.tag = formCollection[inputName].parentNode!.querySelector('.date-msg-error') as HTMLParagraphElement;
      }
  ...
  let inputYear!: number, inputMonth!: number, inputDay!: number, inputDateMs!: number;

  if (!wasError.input || wasError.date) {
    msgError = '';
    // Define the error message element
    if (!MSG_ERROR.date.tag) {
      MSG_ERROR.date.tag = formCollection['date-field'].querySelector(
        ':scope > .date-msg-error',
  ...
  if (wasError.input || wasError.date) {
    ...
    if (outputData.ariaLive !== 'off') {
      ...
      outputArr.forEach((outputName): void => {
